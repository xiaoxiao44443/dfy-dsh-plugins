...
runInNewContext(built.outputFiles[0].text, {
  ...
  require(id) {
    if (id === '@deepseek-ai/dsh-client-ui-primitives') return {
      IconChevronDownOutlineRegular: () => null, IconInspectOutlineRegular: () => null,
      IconSparkleRegular: () => null, Menu: () => null, StateDot: () => null,
    };
    return require(id);
  },
});
...
const sessionBlock = { type: 'plugin:dfy-session-image', version: 1, ref, image };
const attachment = {
...
test('V4 result metadata shows images without resultView or expanding tool details', () => {
...
test('V4 tool content preserves session images and official attachment previews', () => {
  const html = render({ ...settled, content: [sessionBlock, { type: 'image', attachment }] });
  assert.match(html, /aria-label="立绘.png"/);
  assert.match(html, /aria-label="旧图.png"/);
  assert.equal((html.match(/class="dsh-imagegen-thumb"/g) ?? []).length, 2);
});

test('metadata and tool content do not produce duplicate galleries', () => {
...
test('running, failed and malformed results cannot display stale or invalid images', () => {
  for (const block of [
    ...
    { ...settled, isError: true, meta: { images: [{ ref, image }] }, content: [sessionBlock] },
